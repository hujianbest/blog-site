...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    List<Article> findByCategoryIdAndIdNotAndStatusAndDeletedAtIsNullOrderByCreatedAtDesc(
        Long categoryId,
        Long id,
        Article.ArticleStatus status
    );

    @Query("SELECT DISTINCT a FROM Article a JOIN a.tags t WHERE t IN :tags AND a.id != :id AND a.status = :status AND a.deletedAt IS NULL ORDER BY a.createdAt DESC")
    List<Article> findByTagsInAndIdNotAndStatusAndDeletedAtIsNullOrderByCreatedAtDesc(
