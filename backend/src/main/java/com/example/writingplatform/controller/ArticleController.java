...
public class ArticleController {
    ...
    @Operation(summary = "Get related articles by tags or category")
    public ApiResponse<List<ArticleResponse>> getRelated(@PathVariable Long id) {
        List<ArticleResponse> response = articleService.getRelatedArticles(id, 5);
        return ApiResponse.success(response);
    }

    @PutMapping("/{id}/views")
    @Operation(summary = "Increment article view count")
    public ApiResponse<Void> incrementViews(@PathVariable Long id) {
        articleService.incrementViewCount(id);
        return ApiResponse.success(null, "View count updated");
    }
}
