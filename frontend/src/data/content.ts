...
const seedArticles: Article[] = [
  ...
  },
  {
    ...
    tags: [
      { id: "tag-代码心得", name: "代码心得" },
      { id: "tag-tdd", name: "TDD" },
      { id: "tag-ai coding", name: "AI Coding" },
      { id: "tag-软件工程", name: "软件工程" }
    ],
    createdAt: '2026-03-17T00:00:00.000000',
    publishedAt: '2026-03-17T00:00:00.000000',
  }
]
...
export function getLocalArticles(): Article[] {
  ...
  try {
    const value = localStorage.getItem(LOCAL_ARTICLES_KEY)
    return value ? JSON.parse(value) as Article[] : []
  } catch {
    return []
  }
}
...
export function getAllArticles(): Article[] {
  return [...getLocalArticles(), ...seedArticles]
}

export function getPublishedArticles(limit?: number): Article[] {
...
export function getArticleById(id: string | number): Article | null {
  return getAllArticles().find(article => String(article.id) === String(id)) || null
}

export function getCategories(): Category[] {
  return seedCategories.map(category => ({
    ...category,
    articleCount: getPublishedArticles().filter(article => String(article.category?.id) === String(category.id)).length,
  }))
}

export function getCategoryById(id: string | number): Category | null {
  return getCategories().find(category => String(category.id) === String(id)) || null
}

export function getArticlesByCategoryId(id: string | number): Article[] {
  return getPublishedArticles().filter(article => String(article.category?.id) === String(id))
}
...
export function getTags(): Tag[] {
  ...
  return Array.from(counts.values()).map(({ tag, count }) => ({
    ...tag,
    articleCount: count,
  }))
}

export function getArticlesByTagName(name: string): Article[] {
  return getPublishedArticles().filter(article =>
    (article.tags || []).some(tag => tag.name.toLowerCase() === name.toLowerCase())
  )
}

function mergeArticles(articles: Article[]): Article[] {
...
async function fetchRemoteArticles(): Promise<Article[]> {
  if (!isBrowser || import.meta.env.MODE === 'test') {
    return []
  }

  try {
    const response = await fetch(REMOTE_ARTICLES_URL, { cache: 'no-cache' })
    if (!response.ok) {
      return []
    }
...
export async function getAllArticlesAsync(): Promise<Article[]> {
  ...
  return mergeArticles([...getLocalArticles(), ...remoteArticles, ...seedArticles])
}

export async function getPublishedArticlesAsync(limit?: number): Promise<Article[]> {
...
export async function getArticleByIdAsync(id: string | number): Promise<Article | null> {
  return (await getAllArticlesAsync()).find(article => String(article.id) === String(id)) || null
}
...
export async function getCategoriesAsync(): Promise<Category[]> {
  ...
  return seedCategories.map(category => ({
    ...category,
    articleCount: articles.filter(article => String(article.category?.id) === String(category.id)).length,
  }))
}

export async function getCategoryByIdAsync(id: string | number): Promise<Category | null> {
  return (await getCategoriesAsync()).find(category => String(category.id) === String(id)) || null
}

export async function getArticlesByCategoryIdAsync(id: string | number): Promise<Article[]> {
  return (await getPublishedArticlesAsync()).filter(article => String(article.category?.id) === String(id))
}

export async function getTagsAsync(): Promise<Tag[]> {
  const counts = new Map<string, { tag: Tag, count: number }>()

  for (const article of await getPublishedArticlesAsync()) {
    for (const tag of article.tags || []) {
      const key = tag.name
      const current = counts.get(key)
      counts.set(key, { tag, count: (current?.count || 0) + 1 })
...
export async function getArticlesByTagNameAsync(name: string): Promise<Article[]> {
  return (await getPublishedArticlesAsync()).filter(article =>
    (article.tags || []).some(tag => tag.name.toLowerCase() === name.toLowerCase())
  )
}

export function saveLocalArticle(input: ArticleInput): Article {
  const now = new Date().toISOString()
  const localArticles = getLocalArticles()
  const existing = input.id
    ? localArticles.find(article => String(article.id) === String(input.id))
    : undefined
  const article: Article = {
    ...
    categoryName: seedCategories[1].name,
    tags: [{ id: 'local-writing', name: 'Writing' }],
  }

  const nextArticles = existing
    ? localArticles.map(item => String(item.id) === String(article.id) ? article : item)
    : [article, ...localArticles]
  writeLocalArticles(nextArticles)
